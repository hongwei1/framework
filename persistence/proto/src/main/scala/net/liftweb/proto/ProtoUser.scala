...
trait ProtoUser {
  ...
  def sendValidationEmail(user: TheUserType) {
    // OBP fork: original built the link from webkit's `S.hostAndPath`; OBP overrides this method
    // entirely, so the proto base uses a relative path (no webkit `S`).
    val resetLink = "/"+validateUserPath.mkString("/")+
  ...
  def sendPasswordReset(email: String) {
    findUserByUserName(email) match {
      case Full(user) if user.validated_? =>
        ...
        val resetLink = passwordResetPath.mkString("/", "/", "/")+urlEncode(user.getUniqueId())
      ...
      case _ => // user not found: nothing to do (message surfaced by caller)
    }
  }
